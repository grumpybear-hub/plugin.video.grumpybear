...
def artPath():
    theme = appearance()
    if theme in ['-', '']: return
    elif condVisibility('System.HasAddon(script.grumpybear.artwork)'):
        return os.path.join(xbmcaddon.Addon('script.grumpybear.artwork').getAddonInfo('path'), 'resources', 'media', theme)


def appearance():
    appearance = setting('appearance.1') if condVisibility('System.HasAddon(script.grumpybear.artwork)') else setting('appearance.alt')
    return appearance


def artwork():
    execute('RunPlugin(plugin://script.grumpybear.artwork)')
