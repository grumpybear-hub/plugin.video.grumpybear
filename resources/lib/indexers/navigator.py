...
class navigator:
    ...
    def news(self):
        AddonVersion = control.addon('plugin.video.grumpybear').getAddonInfo('version')
    ...
    def tools(self):
        ...
        self.addDirectoryItem("[B]GRUMPYBEAR[/B]: Bibliothek", 'libraryNavigator', 'tools.png', 'DefaultAddonProgram.png')
        self.addDirectoryItem("[B]GRUMPYBEAR[/B]: Anzeige-Typen", 'viewsNavigator', 'tools.png', 'DefaultAddonProgram.png')
        if control.setting('FaultLogger.enabled') == 'true':
            self.addDirectoryItem("[B]GRUMPYBEAR[/B]: Fehlerhafte Provider anzeigen", 'showFaultyProvider', 'tools.png', 'DefaultAddonProgram.png')
        self.addDirectoryItem("[B]GRUMPYBEAR[/B]: Suchverlauf leeren", 'clearCacheSearch', 'tools.png', 'DefaultAddonProgram.png')
        self.addDirectoryItem("[B]GRUMPYBEAR[/B]: Cache leeren", 'clearCache', 'tools.png', 'DefaultAddonProgram.png')
        self.addDirectoryItem("[B]GRUMPYBEAR[/B]: Indexseiten-Cache leeren", 'clearSources', 'tools.png', 'DefaultAddonProgram.png')
        self.addDirectoryItem("[B]GRUMPYBEAR[/B]: Metadaten-Cache leeren", 'clearCacheMeta', 'tools.png', 'DefaultAddonProgram.png')
        self.addDirectoryItem("[B]GRUMPYBEAR[/B]: Alle Caches leeren", 'clearCacheAll', 'tools.png', 'DefaultAddonProgram.png')
    ...
    def library(self):
        self.addDirectoryItem("[B]GRUMPYBEAR[/B]: Bibliothek aktualisieren", 'updateLibrary&query=tool', 'library_update.png', 'DefaultAddonProgram.png')
        self.addDirectoryItem("[B]GRUMPYBEAR[/B]: Film-Ordner", control.setting('library.movie'), 'movies.png', 'DefaultMovies.png',
                              isAction=False)
        self.addDirectoryItem("[B]GRUMPYBEAR[/B]: TV-Serien-Ordner", control.setting('library.tv'), 'tvshows.png', 'DefaultTVShows.png', isAction=False)
    ...
    def views(self):
        try:
            ...
            select = control.selectDialog([i[0] for i in items], "[B]GRUMPYBEAR[/B]: Anzeige-Typen")
    ...
    def infoCheck(self, version):
        try:
            control.infoDialog('github.com/grumpybear-hub/', "HILFE", time=5000,
                               sound=False)
            return '1'
        except:
            return '1'
